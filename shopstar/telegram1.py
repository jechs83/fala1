...
t1 = collection.find({   "product":{"$regex" : '55"' }, "date":date_now, "best_price":{"$lte":1000, "$ne":0}    })
...
t7= collection.find( {"best_price":{"$lte":600},"date":date_now,"brand":{"$in":[ 
...
t8 =  collection.find( {"web_dsct":{"$gte":50},"date":date_now,"brand":{"$in":[ 
...
pro = [t1,t2,t3,t4,t5,t6,t7,t8]  ## ARREGLO DE LOS QUERYS DE MONGO PARA MANDAR POR TELEGRAM
products = []
...
def auto_telegram():
   ...
   for i,v in enumerate(products):
       ...
       if y:
        ...
        colle.update_one(filter, newvalues)     
