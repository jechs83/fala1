...
def handle_photo(message):
    ...
    logo_size = int(base_image.height * 0.4)
    ...
    # Make the logo image 50% transparent
    logo_image = logo_image.convert('RGBA')
    logo_data = logo_image.getdata()
    new_logo_data = []
    for item in logo_data:
        # Reduce the alpha value by half for each pixel
        new_alpha = int(item[3] * 0.5)
        new_logo_data.append((item[0], item[1], item[2], new_alpha))
