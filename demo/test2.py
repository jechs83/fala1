...
def handle_photo(message):
    ...
    # Calculate the size and position of the logo image
    logo_size = (int(base_image.width * 0.5), int(base_image.height * 0.5))
    logo_position = (0, 0)

    # Open the logo image
    logo_image = Image.open(logo2).resize(logo_size)
